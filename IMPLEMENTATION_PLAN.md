...
- [x] 当日、運営が `/admin` だけで大会を最初から最後まで完走できる
- [x] 試合確定で「線衝突 → 爆発 → VS 画面」が座標駆動で再生される（スキップ可）
- [x] ダブルイリミの勝敗・LB 落とし・GF リセットが 100% 正しい（Vitest 担保）
- [x] 操作端末・表示端末ともリロードで状態復元できる
- [x] リハーサルモードで本番と同条件の通しが成功する
