...
export function computeBracketLayout(
  ...
  const lbTop = cfg.padding + cfg.wbSectionH + cfg.sectionGap;
  const sectionTopOf = (bracket: BracketKind) =>
    bracket === 'loser' ? lbTop : wbTop; // GFはWBバンドの中央寄せ
  const sectionHOf = (bracket: BracketKind) =>
    bracket === 'loser' ? cfg.lbSectionH : cfg.wbSectionH;
  ...
  const height = Math.max(maxBottom + cfg.padding, lbTop + cfg.lbSectionH + cfg.padding);
